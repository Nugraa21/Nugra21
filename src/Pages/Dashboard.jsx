...
const Dashboard = () => {
  ...
  return (
    <ErrorBoundary>
      <div className={`min-h-screen font-sans transition-colors duration-300 ${theme === "light" ? "bg-gray-100 text-gray-900" : "bg-gray-900 text-gray-100"}`}>
        {isMobile && (
          <div className="fixed inset-0 bg-gray-900 bg-opacity-95 flex items-center justify-center z-50">
            <div className="bg-white p-8 rounded-xl shadow-2xl max-w-sm text-center">
              ...
              <p className="mt-2 text-sm text-gray-600">Please use a desktop for the best experience.</p>
              <button
        ...
        <nav className={`sticky top-0 z-50 ${theme === "light" ? "bg-white" : "bg-gray-800"} shadow-md`}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16">
              <div className="flex items-center">
                <button
                  onClick={() => setNavbarOpen(!navbarOpen)}
                  className="md:hidden text-yellow-500 hover:text-yellow-600 mr-4"
                >
                  {navbarOpen ? <FaTimes size={20} /> : <FaBars size={20} />}
                </button>
                <h1 className="text-xl font-bold text-yellow-600">Nugra21 Studio</h1>
              </div>
              <div className="hidden md:flex items-center space-x-4">
                {[
                  ...
                  { tab: "certificates", label: "Certificates", icon: <FaCertificate size={16} /> },
                  { tab: "summary", label: "Summary", icon: <FaChartBar size={16} /> },
                ].map(({ tab, label, icon }) => (
                  <button
                    key={tab}
                    onClick={() => {
                    ...
                    className={`flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg transition-all duration-300 ${
                      (activeTab === tab || (tab === "summary" && showSummary))
                ...
                <button
                  onClick={toggleTheme}
                  className="flex items-center gap-2 px-3 py-2 text-sm font-medium bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all duration-300"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 text-sm font-medium bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all duration-300"
                >
              ...
              <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                {[
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg transition-all duration-300 ${
                      (activeTab === tab || (tab === "summary" && showSummary))
                        ? "bg-yellow-500 text-white"
                        : theme === "light"
                        ? "text-gray-700 hover:bg-yellow-100 hover:text-yellow-600"
                ...
                <button
                  onClick={toggleTheme}
                  className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all duration-300"
                >
                  {theme === "light" ? <FaMoon size={16} /> : <FaSun size={16} />}
                  {theme === "light" ? "Dark" : "Light"}
                </button>
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all duration-300"
                >
                  <FaSignOutAlt size={16} /> Logout
                </button>
              </div>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {error && (
            <div className={`bg-yellow-50 text-yellow-700 p-4 rounded-lg text-sm shadow-md mb-6 ${theme === "dark" && "bg-yellow-900 text-yellow-200"} animate-pulse`}>
              <strong className="font-semibold">Error:</strong> {error}
            </div>
          )}
          {showSummary && (
            <div className={`mb-8 p-6 rounded-xl ${theme === "light" ? "bg-white" : "bg-gray-800"} shadow-lg`}>
              <h2 className="text-xl font-semibold text-yellow-600 mb-4">Data Summary</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  ...
                  <div
                    key={label}
                    className="p-4 bg-yellow-50 dark:bg-yellow-900 rounded-lg shadow-md text-center transition-all duration-300 hover:scale-105"
                  >
                    <h3 className="text-sm font-medium text-yellow-600">{label}</h3>
                    <p className="text-2xl font-bold text-yellow-700 dark:text-yellow-300">{count}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
              <div className="relative">
                <input
                  ...
                  className={`w-full sm:w-64 px-4 py-2 rounded-lg text-sm border focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-all duration-300 ${
              ...
              {activeTab === "projects" && (
                <div className="relative">
                  <select
                    ...
                    className={`w-full sm:w-36 px-4 py-2 rounded-lg text-sm border focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-all duration-300 ${
                      theme === "light" ? "bg-white border-gray-200 text-gray-800" : "bg-gray-800 border-gray-700 text-gray-200"
                    }`}
                  >
                    {categories.map((cat) => (
              ...
              {(activeTab === "comments" || activeTab === "projects" || activeTab === "certificates") && (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-yellow-500 text-white text-sm rounded-lg hover:bg-yellow-600 transition-all duration-300 shadow-md"
                >
                  <FaPlus size={12} /> Add
                </button>
              )}
              {selectedItems.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-red-500 text-white text-sm rounded-lg hover:bg-red-600 transition-all duration-300 shadow-md"
                >
              ...
              <button
                onClick={exportData}
                className="flex items-center gap-2 px-4 py-2 bg-green-500 text-white text-sm rounded-lg hover:bg-green-600 transition-all duration-300 shadow-md"
              >
          ...
          {loading ? (
            ...
            filteredContacts.length === 0 ? (
              <div className={`text-center py-12 rounded-xl ${theme === "light" ? "bg-white" : "bg-gray-800"} shadow-lg`}>
                <p className="text-sm text-gray-500">No contacts found.</p>
              </div>
            ) : (
              <div className={`rounded-xl overflow-hidden ${theme === "light" ? "bg-white" : "bg-gray-800"} shadow-lg`}>
                <table className="w-full text-sm">
                  <thead className={`${theme === "light" ? "bg-gray-100" : "bg-gray-700"}`}>
                    <tr>
                  ...
                  <tbody>
                    {filteredContacts.map((contact, index) => (
                      <tr key={contact.id || index} className={`border-t ${theme === "light" ? "border-gray-100 hover:bg-yellow-50" : "border-gray-700 hover:bg-yellow-900"} transition-all duration-200`}>
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            className="text-red-500 hover:text-red-600 flex items-center gap-1 transition-colors duration-300"
                          >
                            <FaTrash size={12} />
                          </button>
                        </td>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filteredComments.map((comment, index) => (
                      <tr key={comment.id || index} className={`border-t ${theme === "light" ? "border-gray-100 hover:bg-yellow-50" : "border-gray-700 hover:bg-yellow-900"} transition-all duration-200`}>
                        <td className="px-4 py-3">
                          <input
                            type="checkbox"
                        ...
                        <td className="px-4 py-3 flex gap-2">
                          <button
                            ...
                            className="text-yellow-500 hover:text-yellow-600 flex items-center gap-1 transition-colors duration-300"
                          >
                            <FaEdit size={12} />
                          </button>
                          <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredProjects.map((project, index) => (
                  <div
                    ...
                    className={`rounded-xl p-5 ${theme === "light" ? "bg-white" : "bg-gray-800"} shadow-lg hover:shadow-xl transition-all duration-300`}
                  >
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-lg font-semibold text-yellow-600 truncate">{project.Title || "N/A"}</h3>
                      <input
                        type="checkbox"
                    ...
                    <div className="flex justify-between mt-4 text-xs">
                      <div className="flex gap-3">
                        {project.Github && (
                          <a href={project.Github} target="_blank" rel="noopener noreferrer" className="text-yellow-500 hover:text-yellow-600 transition-colors duration-300">
                            GitHub
                          </a>
                        )}
                        {project.Link && (
                          <a href={project.Link} target="_blank" rel="noopener noreferrer" className="text-yellow-500 hover:text-yellow-600 transition-colors duration-300">
                            Live
                          </a>
                        )}
                      </div>
            ...
            filteredCertificates.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredCertificates.map((certificate, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-lg font-semibold text-yellow-600 truncate">{certificate.title || "N/A"}</h3>
                      <input
                        type="checkbox"
                    ...
                    <div className="flex justify-between mt-4 text-xs">
                      {certificate.Link && (
                        <a href={certificate.Link} target="_blank" rel="noopener noreferrer" className="text-yellow-500 hover:text-yellow-600 transition-colors duration-300">
                          View
                        </a>
                      )}
                      <div className="flex gap-3">
          ...
          {formOpen && formData && (
            <div className="fixed inset-0 bg-gray-900 bg-opacity-80 flex items-center justify-center z-50">
              <div className={`w-full max-w-4xl rounded-xl ${theme === "light" ? "bg-white" : "bg-gray-800"} shadow-2xl`}>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      <form onSubmit={submitForm} className="lg:col-span-2 space-y-4">
                        {(formData.collection === "comments" || formData.collection === "projects" || formData.collection === "certificates") && (
                          <div>
                            <label className={`block text-xs font-medium ${theme === "light" ? "text-gray-700" : "text-gray-300"} mb-1`}>ID <span className="text-red-500">*</span></label>
                            <input
                              type="text"
                              name="id"
                              value={formData.id}
                              ...
                              required
                              className={`w-full px-3 py-2 text-sm rounded-lg border focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-all duration-300 ${
                                theme === "light" ? "bg-gray-50 border-gray-200" : "bg-gray-800 border-gray-700"
                              } ${!formData.id.trim() ? "border-red-500" : ""}`}
                            />
                            {!formData.id.trim() && <p className="text-red-500 text-xs mt-1 animate-pulse">ID is required</p>}
                          </div>
                        )}
                        {formData.collection === "comments" && (
                          <>
                            <div>
                              <label className={`block text-xs font-medium ${theme === "light" ? "text-gray-700" : "text-gray-300"} mb-1`}>Name <span className="text-red-500">*</span></label>
                              <input
                                type="text"
                                name="name"
                                value={formData.name}
                              ...
                              />
                              {!formData.name.trim() && <p className="text-red-500 text-xs mt-1 animate-pulse">Name is required</p>}
                            </div>
                            <div>
                              <label className={`block text-xs font-medium ${theme === "light" ? "text-gray-700" : "text-gray-300"} mb-1`}>Comment <span className="text-red-500">*</span></label>
                              <textarea
                                name="message"
                                rows={4}
                                ...
                                onChange={handleFormChange}
                                required
                                className={`w-full px-3 py-2 text-sm rounded-lg border focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-all duration-300 resize-y ${
                                  theme === "light" ? "bg-gray-50 border-gray-200" : "bg-gray-800 border-gray-700"
                                } ${!formData.message.trim() ? "border-red-500" : ""}`}
                              />
                              {!formData.message.trim() && <p className="text-red-500 text-xs mt-1 animate-pulse">Comment is required</p>}
                            </div>
                          </>
                        )}
                        {formData.collection === "projects" && (
                          <>
                            <div>
                              <label className={`block text-xs font-medium ${theme === "light" ? "text-gray-700" : "text-gray-300"} mb-1`}>Title <span className="text-red-500">*</span></label>
                              <input
                                type="text"
                                name="Title"
                              ...
                              />
                              {!formData.Title.trim() && <p className="text-red-500 text-xs mt-1 animate-pulse">Title is required</p>}
                            </div>
                            <div>
                              <label className={`block text-xs font-medium ${theme === "light" ? "text-gray-700" : "text-gray-300"} mb-1`}>Description <span className="text-red-500">*</span></label>
                              <textarea
                                name="Description"
                                rows={3}
                              ...
                              />
                              {!formData.Description.trim() && <p className="text-red-500 text-xs mt-1 animate-pulse">Description is required</p>}
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <button
                                ...
                                className="text-yellow-500 text-xs hover:text-yellow-600 flex items-center gap-1 mt-1 transition-colors duration-300"
                              >
                            ...
                            <div>
                              <label className={`block text-xs font-medium ${theme === "light" ? "text-gray-700" : "text-gray-300"} mb-1`}>Category <span className="text-red-500">*</span></label>
                              <select
                                name="category"
                              ...
                              {!formData.category.trim() && <p className="text-red-500 text-xs mt-1 animate-pulse">Category is required</p>}
                            </div>
                          </>
                        )}
                        {formData.collection === "certificates" && (
                          <>
                            ...
                            <div>
                              <label className={`block text-xs font-medium ${theme === "light" ? "text-gray-700" : "text-gray-300"} mb-1`}>Issuer <span className="text-red-500">*</span></label>
                              <input
                                type="text"
                                name="issuer"
                              ...
                              />
                              {!formData.issuer.trim() && <p className="text-red-500 text-xs mt-1 animate-pulse">Issuer is required</p>}
                            </div>
                            <div>
                              <label className={`block text-xs font-medium ${theme === "light" ? "text-gray-700" : "text-gray-300"} mb-1`}>Date <span className="text-red-500">*</span></label>
                              <input
                                type="text"
                                name="date"
                                value={formData.date}
                              ...
                              />
                              {!formData.date.trim() && <p className="text-red-500 text-xs mt-1 animate-pulse">Date is required</p>}
                            </div>
                            <div>
                      ...
                      {(formData.collection === "projects" || formData.collection === "certificates") && (
                        <div className="lg:col-span-1">
                          ...
                          <div className={`rounded-xl p-4 ${theme === "light" ? "bg-gray-50" : "bg-gray-800"} shadow-lg transition-all duration-300`}>
                            <h4 className="text-base font-semibold text-yellow-600 mb-2">{formData.Title || formData.title || "Untitled"}</h4>
                            {imagePreview && (
                              <img
                                src={imagePreview}
                                alt="Preview"
                            ...
                            <div className="flex gap-3 mt-3 text-xs">
                              {formData.Github && (
                                <a href={formData.Github} target="_blank" rel="noopener noreferrer" className="text-yellow-500 hover:text-yellow-600 transition-colors duration-300">
                                  GitHub
                                </a>
                              )}
                              {formData.Link && (
                                <a href={formData.Link} target="_blank" rel="noopener noreferrer" className="text-yellow-500 hover:text-yellow-600 transition-colors duration-300">
