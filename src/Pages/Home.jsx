...
const CTAButton = memo(({ href, text, icon: Icon }) => (
  <a href={href} className="relative group inline-block focus:outline-none focus:ring-2 focus:ring-orange-400 rounded-lg" target="_blank" rel="noopener noreferrer">
    <button className="relative w-40 xs:w-44 sm:w-48 h-10 sm:h-11 rounded-lg border border-orange-300 shadow-md overflow-hidden transition-transform duration-300 hover:scale-105 hover:shadow-lg active:scale-95 flex items-center justify-center gap-2 text-orange-800 font-semibold text-xs sm:text-sm">
      ...
      <span className="relative flex items-center justify-center gap-2">
        {text} {Icon && <Icon className="w-4 h-4" />}
      </span>
    </button>
  </a>
));
...
const SocialLink = memo(({ icon: Icon, link }) => (
  <a href={link} target="_blank" rel="noopener noreferrer" className="relative group inline-block p-2 xs:p-3 rounded-xl bg-orange-100 shadow-md hover:scale-110 hover:bg-orange-200 transition-transform duration-300 focus:outline-none focus:ring-2 focus:ring-orange-400">
...
const TECH_ICONS = [
  { icon: SiReact, name: "React" }, { icon: SiMqtt, name: "MQTT" }, { icon: SiEspressif, name: "ESP32" },
  { icon: SiTailwindcss, name: "TailwindCSS" }, { icon: SiHtml5, name: "HTML5" }, { icon: SiCss3, name: "CSS3" },
  { icon: SiJavascript, name: "JavaScript" }, { icon: SiGit, name: "Git" }, { icon: SiGithub, name: "GitHub" },
  { icon: SiNodedotjs, name: "NodeJS" }, { icon: SiFlutter, name: "Flutter" }, { icon: SiDart, name: "Dart" },
  { icon: SiFirebase, name: "Firebase" }, { icon: SiLatex, name: "LaTeX" }, { icon: SiPhp, name: "PHP" },
  { icon: SiPython, name: "Python" }, { icon: SiVuedotjs, name: "Vue" }, { icon: SiVercel, name: "Vercel" },
];
...
const ScrollingTechText = () => (
  <div className="scrolling-tech-wrapper px-2 xs:px-4 sm:px-6 mt-8 sm:mt-12 w-full overflow-hidden">
    <div className="scrolling-tech-text inline-flex items-center gap-4 xs:gap-6 sm:gap-8 lg:gap-12 text-orange-600 select-none whitespace-nowrap">
      {[...TECH_ICONS, ...TECH_ICONS].map(({ icon: Icon, name }, i) => (
...
const Home = () => {
  ...
  useEffect(() => {
    AOS.init({ once: true, duration: window.innerWidth < 640 ? 600 : 800, easing: "ease-in-out" });
  }, []);
  ...
  return (
    <section className="min-h-screen text-orange-800 flex flex-col items-center justify-center px-2 xs:px-4 sm:px-6 md:px-8 lg:px-12 pt-20 sm:pt-24 pb-12 sm:pb-16 overflow-x-hidden relative" id="Home">
      ...
      <div className="md:hidden mt-8 mb-4 flex justify-center">
        <img src="/profile.jpg" alt="Profile" className="w-29 h-29 rounded-full shadow-lg border-4 border-orange-400 object-cover" />
      </div>

      <div className="max-w-7xl w-full flex flex-col items-center lg:items-start gap-6 xs:gap-8 sm:gap-12 lg:gap-16">
        <div className="flex flex-col space-y-6 xs:space-y-8 w-full lg:w-1/2">
          <MainTitle />
          <div className="h-8 xs:h-9 flex items-center font-semibold text-orange-700 text-lg xs:text-xl sm:text-2xl tracking-wide" data-aos="fade-up" data-aos-delay="800">
            <span>{text}</span>
